...
def simulate_full(sim_folder, increments, f0, bw, Nf, f_samp, fold, subintlen, period, flux, psr_name,\
    ...
    # Define the ISM object
        ...
        if f0 > 500.0 and 'puppi' in tempfits:
            ...
            sim_signal._data = sim_signal.data[:,:]/np.sqrt(2000.0) #Line we are modifying for noise
            # Get our template PSRFITS file
            fitspath = sim_folder+'/'+'fits_Lband_'+str(ii)+'.fits'
...
dm = 9.022146 # pc cm^-3
F0 = np.double(190.2678373613727) # pulsar frequency, Hz
period = np.double(1.0)/F0 # pulsar period, seconds
bw_Lband = 800.0 # bandwidth, MHz
#WE WILL WANT TO EDIT THIS NUMBER
Nf_Lband = int(sys.argv[2]) # number of frequency channels
f0_Lband = 1380 # central observing frequency, MHz
telescope = "gbt" # Telescope name (for default telescopes)"GBT" or "Arecibo"
psr_name = "J1012+5307" # pulsar name
flux_Lband = 0.0000170 # mean flux (AO) for GB it is 0.00202 Jy (from Alam et al. 2020)
...
def combine_tim(timfiles, outfile = "J0931-1902"):
    """
...
tempfits_Lband = "/bowser/jdc0059/SimulatorProj/templatefiles/" + sys.argv[1] + "/guppi_57394_J1012+5307_0022_0001.fits"
#tempfits_Lband = sys.argv[4]
...
fitsfiles_Lband = simulate_full(sim_folder, increments_Lband_all, f0_Lband, bw_Lband, Nf_Lband, f_samp,True, subintlen_Lband, period, flux_Lband, psr_name, ObsTime_Lband, All_Lband_DMs_const, tempfits_Lband, parfile, temp_Lband_ar, twoD = False, alpha = -1.56, ref_MJD=56000.0, MJD_start=56000.0)


# Get the 1400 MHz TOAs
timfile_Lband = getSimTOAs(fitsfiles_Lband, temp_Lband, scrunch = False, nchan = 64, nsubint = 1, npol = 1)
